	...
	<body>
		...
		<div id="m-melanin-tab-header">
			<g:render plugin="melanin" template="/templates/m-melanin-action-bar"
				model="${[
						buttons:[[name:'m-melanin-action-bar-button-create-node',label:'Tạo Lỗi']]
					]}"/>
			<g:render plugin="melanin" template="/templates/m-melanin-breadcrum"
				model="${[
						items:[[href:createLink(controller:'admin'),title:'OpRisk',label:'OpRisk'],
						[href:createLink(controller:'admin',action:'viewError'),title:'Danh sách lỗi',label:'Danh sách Lỗi']]
					]}"/>
		...
		<div id="m-melanin-main-content">
			...
			<div id="m-oprisk-tree-section" style="float:left;max-width:350px;width:350px">
				<fieldset>
					<legend>Danh sách các lỗi</legend>
			...
			<div id="m-oprisk-form-section" style="float:left;margin-left:200px;display:none">
				<fieldset style="width:350px;">
					...
					<form id="m-oprisk-tree-node-form" name="m-oprisk-tree-node-form" controller="admin" action="addError">
						<ul class="form form-clear form-stacked">
							<li>
								<label for="name"><font color="red">*</font> Lỗi:</label>
		...
		<script type="text/javascript">
			$(function(){
				...
				$("#m-oprisk-tree").jstree({
					"json_data" : {
						"ajax" : { "url" : "${createLink(controller:'admin',action:'getErrorTree')}","cache": false }
	                },
	                "themes" : {
						"theme":"msb"
					},
        		...
        		$("button[name=m-melanin-form-button-delete]").click(function(){
					jquery_confirm("Xóa lỗi","Anh/chị chắc chắn muốn xóa lỗi này?",
							function(){
								jquery_open_load_spinner();
								document.location = "${createLink(controller:'admin',action:'deleteError')}/" + $('form[name=m-oprisk-tree-node-form] input#nodeId').val() + "?currentAction=" + $('form[name=m-oprisk-tree-node-form] input#currentAction').val();
							});
				});
...
//        			$("form[name=m-oprisk-tree-node-form] select[name=parent]").val("");
				...
				$("#admin-error").closest('li').addClass('active');
				set_side_bar(true);
			});
