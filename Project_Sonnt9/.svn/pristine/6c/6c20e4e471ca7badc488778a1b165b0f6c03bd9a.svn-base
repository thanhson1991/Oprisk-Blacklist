...
// Blacklist quản lý danh sách cá nhân
	def blacklistQldsCnDisplay(ArrayList<String> content){
		...
		def date = new Date()
		content.each{
			subcontent += messageSource.getMessage('BlackIndividual.blacklistQldsCnDisplay.SubContent',[it[0],it[1],it[2]].toArray(),LCH.getLocale())
		}
	...
	def blacklistQldsPnDisplay(ArrayList<String> content){
		...
		def date = new Date()
		content.each{
			subcontent += messageSource.getMessage('BlacklistCorporation.blacklistQldsPnDisplay.SubContent',[it[0],it[1],it[2]].toArray(),LCH.getLocale())
		}
	...
	def blacklistQldsTsbdDisplay(ArrayList<String> content){
		...
		def date = new Date()
		content.each{
			subcontent += messageSource.getMessage('BlacklistTSBD.blacklistQldsTsbdDisplay.SubContent',[it[0],it[1],it[2]].toArray(),LCH.getLocale())
		}
