...
<%-- </g:else>--%>
   ...
   <li class="allTop">
		...
		<g:select style="width:120px" name="LoiCap1" id="LoiCap1" from="${allErrorList1}"
							optionKey="id" optionValue="${{it.code+'-'+it.name} }" noSelection="${['':'']}"							
							value="${LoiCap1}" /> 
							
   	    <g:select style="width:120px" name="LoiCap2" id="LoiCap2" from="${allErrorList2 }" optionKey="id" 
   	    					 optionValue="${{it.code+'-'+it.name} }" value="${LoiCap2}"
   	     					noSelection="${['':'']}" />
   	    
   	    <g:select name="LoiCap3" id="LoiCap3" style="width:120px" from="${ allErrorList3}"
							optionKey="id" optionValue="${{it.code+'-'+it.name} }" value="${LoiCap3}" noSelection="${['':'']}" />   
   	    	
<%--		<g:select name="LoiCap3" id="LoiCap3" style="width:120px" from="${allErrorList3}"--%>
<%--							optionKey="id" />--%>
		<g:submitButton class="searchButtons btn primary" style="margin-left:0px;margin-top:0px;width:100px" name="search" value="Lọc thông tin" />
	</li>
<!-- Lable Hàng 2 -->
   <li>
   ...
   <li class="allTop">
   		<g:select style="width:155px" name="NHCD" id="NHCD" from="${allDonViNhap1}"
							value="${tenDonVi1 }" optionKey="id" optionValue="${{it.code+'-'+it.name} }" noSelection="${['':'']}" />
							
  		<g:select  name="CN" id="CN" style="width:120px" from="${ allDonViNhap2}"
  				 optionKey="id" optionValue="${{it.code+'-'+it.name} }" value="${tenDonVi2 }" noSelection="${['':'']}" />			

   		<g:select name="PGD" id="PGD" style="width:120px" from="${allDonViNhap3 }" optionKey="id" optionValue="${{it.code+'-'+it.name} }" value="${tenDonVi3 }" noSelection="${['':'']}"/>
     	<g:select style="width:120px" name="HTPH" from="${allErrorCheck}"
							optionKey="id" optionValue="${{it.code+'-'+it.name} }" noSelection="${['':'']}" value="${erroCheck}" /> <!--ErrorCheck-->
		<g:select style="width:120px" name="LoaiNghiepVu" value = "${LoaiNghiepVu }" id="LoaiNghiepVu" from="${allbusiness}"
							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}"/>	
...
<!-- Lable Hàng3 -->   	
   ...
   <li class="allTop">   		   
   		<g:select style="width:155px" name="NHCDLoi" id= "NHCDLoi" from="${allDonViLoi1}" 
							value="${DonViLoi1}" optionKey="id" optionValue="${{it.code+'-'+it.name} }" noSelection="${['':'']}" />						
   		
   		<g:select name="CNLoi" id="CNLoi" from="${allDonViLoi2 }"
   				optionKey="id"  optionValue="${{it.code+'-'+it.name} }" value="${DonViLoi2 }"  noSelection="${['':'']}" style="width:120px"/>
   		
   		<g:select  name="PGDLoi" id="PGDLoi" style="width:120px" from="${ allDonViLoi3}" 
   	      					optionKey="id" optionValue="${{it.code+'-'+it.name} }" value="${DonViLoi3 }" noSelection="${['':'']}" />
   		
   		 
   		
   		
   		<input type="text" name="NguoiGayLoi" style="width:110px" value="${NguoiGayLoi }" />		
		<g:select style="width:120px" name="TrangThai" value="${TrangThai }" id="TrangThai" from="${allErrorStatus}"
