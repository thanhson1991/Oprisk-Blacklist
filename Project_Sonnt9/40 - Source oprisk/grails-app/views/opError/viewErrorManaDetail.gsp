...
<%--   	      <br>--%>
<%--   	       <g:select class="validate[required] largeControlCenter largeControlCenterSelect disableCombobox" name="NHCD" from="${department}"--%>
...
<%--   </li>--%>
	...
	<table class="tableControlCenter">
		<tr>
			<td><label for="TenDonVi3" class="lableTableCenter ">PGD/Phòng/Nhóm gây lỗi<font color="red">*</font> </label></td>
			<td><label for="TenDonVi2" class="lableTableCenter ">CN/Trung tâm/Phòng<font color="red">*</font> </label></td>
			<td><label for="TenDonVi1" class="lableTableCenter ">NH Chuyên doanh/Khối<font color="red">*</font> </label></td>
		</tr>
		<tr>
			<td><g:select  name="TenDonVi3" from="${allUnitDepart3}" class="largeControlCenter controlComboboxTableCenter validate[required] disableCombobox"
   	      					value = "${errorManagement?.tenDonVi3 }"
							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}" /></td>
			<td><g:select  name="TenDonVi2" from="${unitDepart2}" class="largeControlCenter controlComboboxTableCenter validate[required] disableCombobox"
   	      					value = "${errorManagement?.tenDonVi2 }"
							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}" /></td>
			<td><g:select  name="TenDonVi1" from="${unitDepart1}" class="largeControlCenter controlComboboxTableCenter validate[required] disableCombobox"
	...
	<li>
 		<table class="tableControlCenter">
 			...
 			<tr style="border:0px ">
 				<td style="border:0px"><g:select class="controlComboboxTableCenter disableCombobox validate[required]" name="errorCheck" from="${errorCheck}"
							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}"
							value="${errorManagement?.errorCheck?.id}" /></td>
 				<td style="border:0px"><input type= "text" id = "SoLuongKiemTra" name="SoLuongKiemTra" class = "disableControl controlTableCenter onlyNumber2 textRight " value="${errorManagement?.soLuongKiemTra }"></input></td>
 				<td style="border:0px"><input type= "text" id = "TongSoChonMau" name="TongSoChonMau" class = "controlTableCenter disableControl price" value="${errorManagement?.tongSoChonMau }"></input></td>
	...
	<sec:ifAnyGranted roles="ROLE_GDTT">
		...
		<%-- <g:hiddenField name="hiddenFieldDonVi" value="${user.prop4?user.prop4:0}"/>		 --%> 		
   </sec:ifAnyGranted>
...
<%--   </li>  --%>
   ...
   --%><%--
    ...
    <li>
   	       <label for="TenDonVi" class="lableCenter ">Nhập tên đơn vị<font color="red">*</font> </label>
   	      <br>
   	      <g:select class="validate[required] largeControlCenter" name="TenDonVi" from="${unitDepart}"
    ...
    --%><li>
	   	...
	   	<br>
	   		...
	   		<tr>
	   			<td><input type="text" id="MaGiaoDich" name="MaGiaoDich" class="disableControl controlTableCenter" value="${errorManagement?.maGiaoDich}" ></input></td>
	   			<td><input type="text" id="GiaTriGiaoDich" name="GiaTriGiaoDich" class="disableControl controlTableCenter onlyNumber textRight" value="${errorManagement?.giaTriGiaoDich }"></input></td>
	   			<td><input type="text" id="LoaiTien" name="LoaiTien" value="${errorManagement?.loaiTien }" class="disableControl controlTableCenter"></input></td>
	   		</tr>
	   		<tr>
	   		...
	   		<tr>
	   			<td><input type="text" id="SoCifKhachHang" name="SoCifKhachHang" class="disableControl controlTableCenter" value="${errorManagement?.soCifKhachHang }"></input>    </td>
	 ...
	 <li style="display:None">
  		...
  		<select id="loaiNghiepVu" class="largeControlCenter"></select>  		
   	</li>
    ...
    <li>
   	      ...
   	      <br>
   	      	<g:select class="largeControlCenter largeControlCenterSelect disableCombobox validate[required]" name="errorCategory" from="${errorCategory}"
							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}"
    ...
    <li>
    	<table class="tableControlCenter" style="border:0px !important">
    		...
    		<tr style ="border:0px">
				<td style ="border:0px"><g:select class=" validate[required] largeControlCenter disableCombobox controlComboboxTableCenter" name="LoiCap3" from="${allErrorList3}"
   							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}"							 
							value="${errorManagement?.loiCap3}" /></td>    			
				<td style ="border:0px"><g:select class=" validate[required] largeControlCenter disableCombobox controlComboboxTableCenter" name="LoiCap2" from="${errorlist2}"
   							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}"							 
							value="${errorManagement?.loiCap2}" /></td>
		
				<td style ="border:0px"><g:select class=" validate[required] largeControlCenter disableCombobox controlComboboxTableCenter" name="LoiCap1" from="${errorlist1}"
   							optionKey="id" optionValue="${{it.code+'-'+it.name}}" noSelection="${['':'']}"							 
							value="${errorManagement?.loiCap1}" /> </td>																	
    		</tr>
    	</table>       	        
    <li>
    ...
    <li>
    	<table class="tableControlCenter" style="border:0px !important">
    		...
    		<tr style ="border:0px">
				...
				<td style ="border:0px"><g:textArea name="MotaChiTiet" value="${errorManagement?.motaChiTiet}" class="largeControlTextCenter disableControl validate[required]" rows="2" id="MotaChiTiet"/></td>
    ...
    <br>
     ...
     <li>
   	      ...
   	      <br>
   	      <g:select class="validate[required] largeControlCenter largeControlCenterSelect disableCombobox" name="TrangThai" from="${errorStatus}"  value="${errorManagement?.trangThai}"
    ...
    <li>
    <g:if test="${errorManagement?.errorUserCreate}">
       <g:each in="${errorManagement?.errorUserCreate}" var="d" status="t">
    	<li id="runDate_${t+1}" >
    		...
    		<tr>
    			...
    			<td rowspan="4">
    				  <a class="deleteDate deleteUncreatedDate disableControl set-edit" tid="1" title="Xoa ngay" href="#body"> <img  border="0" src="${resource(dir:'images',file:'icon_delete.PNG')}" height="15" alt="" /></a>
    		...
    		<tr>
    			...
    			<td><input class="errorMucDoLoi disableControl"  name ="Mucdoloi_${t+1}" id="Mucdoloi_${t+1}" value="${d.levelError}" width="100px" style="!important" /></td>
    			  
    			<td><input class="errorBDSUser"  name ="BdsUser_${t+1}" id="BdsUser_${t+1}" value="${d.bDSUser}"  readonly="readonly" />         </td>
    			<td><input class="errorNhanSu"   name ="IdNhanSu_${t+1}" id="IdNhanSu_${t+1}" value="${d.codeSalary}" width="100px"  readonly="readonly"/>   </td>
    		...
    		<tr>
    			...
    			<td> <label for="PGD" class="PGD">PGD/Phòng ban/Tổ Nhóm ${t+1}</label></td>
    		</tr>
  ...
  <g:else>
	...
	<li id="runDate_1" >
       	<table class="tableControlCenter">
    		<tr>
    			<td> <label for="OutLook_1" class="nguoiso">11.1 UserOutlook 1</label> </td>
    			<td> <label for="HoVaTen_1" class="hovaten">11.2 Họ và tên 1</label></td>
    			<td> <label for="ChucDanh_1" class="chucdanh">11.3 Chức danh 1</label></td>
    			<td rowspan="4">
    		...
    		<tr>
    			...
    			<td><input style="width:177px !important" class="errorManagement"  tid="1" name ="OutLook_1" id="OutLook_1" /> </td>    			
    			<td> <input class="errorHoVaTen"  name ="HoVaTen_1" id="HoVaTen_1" readonly="readonly" /></td>
    				
    			<td> <input class="errorChucDanh"  name ="ChucDanh_1" id="ChucDanh_1"  readonly="readonly" /></td>
    		...
    		<tr>
    			...
    			<td><input class="errorMucDoLoi"  name ="Mucdoloi_1" id="Mucdoloi_1"  width="100px" style="!important" /></td>
    			  
    			<td><input class="errorBDSUser"  name ="BdsUser_1" id="BdsUser_1"   />         </td>
    			<td><input class="errorNhanSu"  name ="IdNhanSu_1" id="IdNhanSu_1"  />   </td>
    			
    		</tr>
    		<tr>
    			<td> <label for="NHCD_1" class="NHCD">NHCD/Khối hỗ trợ 1</label></td>
...
<%--                    <a style="margin-left:-250px !important"  href="${resource(dir:'errorFiles',file:errorManagement.fileName)}" >${errorManagement.fileName }</a>--%>
   ...
   <li>
    ...
    <g:if test="${isSave=='Save'}">
		<button class="btn primary" id="saveError" type="button" name="saveError" style="margin-bottom:25px" value="saveError">Cập nhật</button>
		...
		<g:hiddenField name="actionButton" id="actionButton" value=""/>	
    </g:if>
<%--    ${ isSaveStatus}--%>
...
<%--  		<button class="btn primary" style="margin-bottom:5px;margin-top:5px" id="saveStatus" name="saveStatus" value="saveStatus">Cập nhật trạng thái</button>--%>
   ...
   <sec:ifAnyGranted roles="ROLE_GDTT_LEVEL3,ROLE_GDTT_LEVEL4">
      <g:if test="${isSaveStatus != 'Yes' && isSave != 'Save'}">
		<button class="btn primary" id="saveError" type="button" name="saveError" style="margin-bottom:25px" value="saveError">Cập nhật</button>
		<button class="btn primary" type="submit" id="dupplicateError" name="dupplicateError" value="dupplicateError">Lưu thành lỗi mới</button>
		<g:hiddenField name="actionButton" id="actionButton" value=""/>	
	  </g:if>
