...
<html>
  ...
  <body>
	...
	<g:form method="post" name="insertErrorCheck" controller="admin" action="errorCheckUpdateDelete" enctype="multipart/form-data">
 ...
 <g:if test="${flash.message}">
  ...
  <ol class="form form-clear olCenter" id="incidentField">
   <li>
   	      ...
   	      <br>
   	       <input type="text" value="${errorCheck.name }"  name ="checkName" id="checkName"   class="  text-input largeControlCenter">
   </li>
   
    <li>
   	     <label for="checkCode" class="lableCenter ">Mã kiểm tra  </label>
   	      <br>
   	      <input type="text" value="${errorCheck.code }"  name ="checkCode" id="checkCode"   class=" text-input largeControlCenter">
   </li>  
     
    
   <li> 
   	<button class="btn primary" type="submit" value="saveCheck">Sửa</button> 
   	<button class="btn primary"id="delete" value="deleteCheck">Xóa</button>
   	<g:hiddenField id="errorCheckId" name="errorCheckId" value="${errorCheck.id}"/>
