				...
				<fieldset style="width:400px">
					...
					<form id="m-oprisk-tree-node-form"  name="m-oprisk-tree-node-form" controller="unitDepartment" action="addNode">
						<ul class="form form-clear form-stacked">
							...
							<li>
								<label for="code"><font color="red">*</font> Mã lỗi:</label>
		...
		<script type="text/javascript">
			$(function(){
				...
				$("#m-atrack-tree").jstree({
					"json_data" : {
						"ajax" : { "url" : "${createLink(controller:'errorAdmin',action:'getTree')}/horizontal" }
	                },
	                "themes" : {
						"theme":"msb"
					},
        		...
        		$("#m-atrack-tree a").live("click",function(){
        			...
        			//$("#AddMaLoi").val($("form[name=m-oprisk-tree-node-form] input#code").val());
        			$("#AddChildrenId").val($("form[name=m-oprisk-tree-node-form] input#nodeId").val());
        			/*alert($("#AddChildrenId").val());*/
					       			
        			
        			$.post('${createLink(controller:'errorAdmin',action:'findParent')}/AddChildrenId',
							$("form[name=m-oprisk-tree-node-form]").serialize(),function(data){								
								$("#AddTrucThuoc").val(data);
								
		               	});
        			$.post('${createLink(controller:'errorAdmin',action:'findCodeParent')}/AddChildrenId',
							$("form[name=m-oprisk-tree-node-form]").serialize(),function(data){								
								$("#AddMaLoi").val(data);
        		...
        		$("button[name=m-melanin-form-button-delete]").click(function(){
					jquery_confirm("Xóa đơn vị","Bạn chắc chắn muốn xóa đơn vị này?<br/><br/><b>Lưu ý:</b> Tất cả các đơn vị con của đơn vị này cũng sẽ bị xóa!",
							function(){
								...
								document.location = "${createLink(controller:'errorAdmin',action:'deleteNode')}/" + $('form[name=m-oprisk-tree-node-form] input#nodeId').val() + "?currentAction=" + $('form[name=m-oprisk-tree-node-form] input#currentAction').val();
							});
				});
