	...
	<body>
		...
		<div id="m-melanin-tab-header">
			<g:render plugin="melanin" template="/templates/m-melanin-action-bar"
				model="${[
						buttons:[[name:'m-melanin-action-bar-button-create-node',label:'Tạo loại nghiệp vụ']]
					]}"/>
			<g:render plugin="melanin" template="/templates/m-melanin-breadcrum"
				model="${[
						items:[[href:createLink(controller:'admin'),title:'OpRisk',label:'OpRisk'],
						[href:createLink(controller:'admin',action:'viewDepartment'),title:'Danh sách loại nghiệp vụ',label:'Danh sách loại nghiệp vụ']]
					]}"/>
		...
		<div id="m-melanin-main-content">
			...
			<div id="m-oprisk-tree-section" style="float:left;max-width:300px;width:300px">
				<fieldset>
					<legend>Danh sách các loại nghiệp vụ</legend>
			...
			<div id="m-oprisk-form-section" style="float:left;margin-left:20px;display:none">
				<fieldset style="width:300px">
					...
					<form method="post" id="m-oprisk-tree-node-form" name="m-oprisk-tree-node-form" controller="admin" action="addDepartment">
						<ul class="form form-clear form-stacked">
							<li>
								<label class="e-xxl" for="name"><font color="red">*</font> Tên loại nghiệp vụ:</label>
		...
		<script type="text/javascript">
			$(function(){
				set_side_bar(true);
                                $("#admin-department").closest('li').addClass('active');
				...
				$("#m-oprisk-tree").jstree({
					"json_data" : {
						"ajax" : { "url" : "${createLink(controller:'admin',action:'getDepartmentTree')}","cache": false }
	                },
	                "themes" : {
						"theme":"msb"
					},
...
//                          $.get('${createLink(action:'getDepartment',controller:'admin')}/'+
//						$(this).closest('li').attr('id'),function(data){
        		...
        		$("button[name=m-melanin-form-button-delete]").click(function(){
					jquery_confirm("Xóa loại nghiệp vụ","Anh/chị chắc chắn muốn xóa loại nghiệp vụ này?<br/>",
							function(){
								jquery_open_load_spinner();
								document.location = "${createLink(controller:'admin',action:'deleteDepartment')}/" + $('form[name=m-oprisk-tree-node-form] input#nodeId').val() + "?currentAction=" + $('form[name=m-oprisk-tree-node-form] input#currentAction').val();
							});
				});
