...
<html>
  ...
  <body>
           ...
           <div id="m-melanin-tab-actions">
			...
			<g:render plugin="melanin" template="/templates/m-melanin-breadcrum"
				model="${[
						...
						[href:createLink(controller:'opRisk',action:'report'),title:'Tổng hợp đánh giá tình hình QLRR hiện tại',label:'Tổng hợp đánh giá tình hình QLRR hiện tại']]
					]}"/>
           </div>
			<div class="clear"></div>
		</div>
    ...
    <div id="m-melanin-main-content">
     <g:form name="reportForm" class="form" action="report">
	        ...
	        <tr>
	          <td >${p.branchName}</td>
	           <g:each var="g" in="${0..19}">
                  <td class ="center">${RiskService?.getAnswerScore(p?.surveyInstance?.answers?.id[g])}</td>                 
               </g:each>
                  <%def totalScore = 0%>
                  <g:each var="k" in="${0..(p.surveyInstance.answers.size()-1)}">
                    <g:if test="${p.surveyInstance.answers.answer[k]!=null}">
                      <%totalScore = totalScore + RiskService?.getAnswerScore(p?.surveyInstance?.answers?.id[k]).toInteger();%>
                     </g:if>
                  </g:each>
                  ...
                  <td class ="center"><a href="${createLink(controller:'opRisk',action:'create',params:[id:p.id])}" >Xem</a></td>
                  </g:each>
    ...
    $(document).ready(function(){
       $("#baro-report").closest('li').addClass('active');
      set_side_bar(true);
        });
